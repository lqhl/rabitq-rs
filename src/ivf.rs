        ...
        for batch_idx in 0..total_batches {
            ...
            for i in 0..actual_batch_size {
                ...
                if self.ex_bits > 0 {
                    ...
                    //
                    // MEMORY OPTIMIZATION: Unpack ex_code on-demand (only after lower-bound check)
                    // C++ unpacks in split_distance_boosting, not beforehand
                    // Saves 4.4x memory (GIST1M: 3,976 MB → 904 MB)

                    // Unpack ex_code on-demand (C++-style)
                    let ex_code_packed = &cluster.ex_codes_packed[global_idx];
                    let ex_code_unpacked =
                        unpack_ex_code(ex_code_packed, self.padded_dim, self.ex_bits as u8);
